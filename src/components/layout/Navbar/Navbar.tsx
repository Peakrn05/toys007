          ...
          <div className="flex-1 max-w-xl hidden md:flex">
            <div className="relative w-full">
              <input
                ...
                className="w-full border-2 border-gray-200 rounded-lg pl-4 pr-12 py-2 text-sm focus:outline-none focus:border-brand-red transition-colors"
              />
              <button className="absolute right-0 top-0 h-full px-4 bg-brand-red text-white rounded-r-lg hover:bg-brand-red-dark transition-colors">
                <Search size={18} />
              </button>
            </div>
          </div>
          ...
          <ul className="flex items-center gap-0">
            {NAV_LINKS.map((link) => (
              ...
              >
                <Link
                  href={link.href}
                  className="flex items-center gap-1 px-4 py-3 text-sm font-medium text-white hover:bg-brand-blue-dark transition-colors"
                >
                  {link.label}
                ...
                {link.children.length > 0 &&
                  activeDropdown === link.label && (
                    <div className="absolute top-full left-0 bg-white shadow-lg rounded-b-lg border border-gray-100 min-w-44 z-50">
                      <ul className="py-2">
                        {link.children.map((child) => (
                          <li key={child}>
                            <Link
                              href={`${link.href}/${child.toLowerCase().replace(/ /g, "-")}`}
          ...
          <div className="px-4 py-3 border-b border-gray-100">
            <div className="relative">
              <input
                ...
                className="w-full border-2 border-gray-200 rounded-lg pl-4 pr-10 py-2 text-sm focus:outline-none focus:border-brand-red"
              />
              <Search
                size={16}
          ...
          <ul className="py-2">
            {NAV_LINKS.map((link) => (
              <li key={link.label}>
                <Link
                  href={link.href}
                  className="block px-4 py-3 text-sm font-medium text-gray-700 hover:bg-gray-50 hover:text-brand-red transition-colors"
